...
class PostController extends Controller
{
    ...
    {
        if (1/* IS_Python*/) {
            //dd(base_path().'\app\Python\hello.py');
            // $process = new Process(
            //     ['python', public_path() . '/python.py'], null,
            //     ['SYSTEMROOT' => getenv('SYSTEMROOT'), 'PATH' => getenv("PATH")]
            // );
            //$python = env('python'); dd($python);
            $scripts_path   = base_path() . '\app\Python\\';
            $script_name    = "hello.py";
            $process      = new Process(['python', $scripts_path . $script_name]);
            $process      = new Process(['python',  '-V']);
            $process      = Process::fromShellCommandline('python ' . $scripts_path . $script_name);
            //$process      = new Process(['python',  $scripts_path . 'parent.py']); 
